##This program is mainly just to find how Fringe affects sales of products
##The idea is to not include weeks when there is Fringe (or any other weeks which were affected
##by any other factors, like holidays, matches or any other), then calculate the equation of
##line of best fit for sales vs temperature graph. After, predict sales by using the temperature
##for the weeks we didn't include and to calculate how fringe, holidays, matches, etc.
##affect sales, use the following formula:
##                  Data(Real) - Data(Predicted)
##                 ------------------------------- x 100%
##                          Data(Predicted)
...
for y in range(len(names)):
    print("%-65s %s" %(names[y], round(Fringe_percentages[y],2)), "%")
